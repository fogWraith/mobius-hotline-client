...
func (s *ServerScreen) Update(msg tea.Msg) (ScreenModel, tea.Cmd) {
	...
	case ServerComposeMessageMsg:
		s.model.handleServerComposeMessageMsg(msg)
		return s, nil
...
func (s *ServerScreen) View() string {
	...
	chatBorderColor := ColorCyan // Default cyan
	...
	if !s.focusOnUserList {
		...
		if !s.chatViewport.AtBottom() {
			chatBorderColor = ColorLightGrey
		}
	}
	...
	return lipgloss.JoinVertical(
		lipgloss.Left,
		serverTitleStyle.Render(fmt.Sprintf("Mobius - Connected to %s", s.serverName)),
		shortcuts,
		lipgloss.JoinHorizontal(
			lipgloss.Top,
			lipgloss.JoinVertical(lipgloss.Left, chatView, boxStyle.Render(s.chatInput.View())),
