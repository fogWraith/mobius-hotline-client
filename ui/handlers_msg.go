...
func (m *Model) resizeAllScreens(w, h int) {
	if m.homeScreen != nil { m.homeScreen.SetSize(w, h) }
	if m.joinServerScreen != nil { m.joinServerScreen.SetSize(w, h) }
	if m.bookmarkScreen != nil { m.bookmarkScreen.SetSize(w, h) }
	if m.trackerScreen != nil { m.trackerScreen.SetSize(w, h) }
	if m.settingsScreen != nil { m.settingsScreen.SetSize(w, h) }
	if m.serverScreen != nil { m.serverScreen.SetSize(w, h) }
	if m.newsScreen != nil { m.newsScreen.SetSize(w, h) }
	if m.newsArticlePostScreen != nil { m.newsArticlePostScreen.SetSize(w, h) }
	if m.newsBundleFormScreen != nil { m.newsBundleFormScreen.SetSize(w, h) }
	if m.newsCategoryFormScreen != nil { m.newsCategoryFormScreen.SetSize(w, h) }
	if m.legacyNewsPostScreen != nil { m.legacyNewsPostScreen.SetSize(w, h) }
	if m.accountsScreen != nil { m.accountsScreen.SetSize(w, h) }
	if m.filesScreen != nil { m.filesScreen.SetSize(w, h) }
	if m.tasksScreen != nil { m.tasksScreen.SetSize(w, h) }
	if m.logsScreen != nil { m.logsScreen.SetSize(w, h) }
	if m.messageBoardScreen != nil { m.messageBoardScreen.SetSize(w, h) }
	if m.filePickerScreen != nil { m.filePickerScreen.SetSize(w, h) }
	if m.composeMessageScreen != nil { m.composeMessageScreen.SetSize(w, h) }
	if m.modalScreen != nil { m.modalScreen.SetSize(w, h) }
}
...
func (m *Model) handleErrorMsg(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	m.modalScreen = NewModalScreen("Error", errorMessage.text, []string{"Close"}, m)
...
func (m *Model) handleServerMsgMsg(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	title := "Private Message from " + serverMessage.from
	content := serverMessage.text + "\n\nAt " + serverMessage.time
	// Store reply target info for when user clicks Reply
	m.replyTargetID = serverMessage.userID
	m.replyTargetName = serverMessage.from
	m.replyQuoteText = serverMessage.text
	m.modalScreen = NewModalScreen(title, content, []string{"Close", "Reply"}, m)
	m.PushScreen(ScreenModal)
	return m, m.modalScreen.Init()
}
...
func (m *Model) handleAgreementMsg(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	m.modalScreen = NewModalScreen("Server Agreement", agreementMessage.text, []string{"Disagree", "Agree"}, m)
	m.PushScreen(ScreenModal)
	return m, m.modalScreen.Init()
}
...
func (m *Model) handleJoinServerConnectMsg(msg JoinServerConnectMsg) tea.Cmd {
	...
	if err != nil {
		m.modalScreen = NewModalScreen("Connection Error", err.Error(), []string{"OK"}, m)
		m.PushScreen(ScreenModal)
...
func (m *Model) handleFileInfoMsg(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	// Display modal
	m.modalScreen = NewModalScreen("File Information", contentBuilder.String(), []string{"Close"}, m)
	m.PushScreen(ScreenModal)
...
func (m *Model) handleServerDisconnectRequestedMsg() tea.Cmd {
	m.modalScreen = NewModalScreen("Disconnect from the server?", "", []string{"Cancel", "Exit"}, m)
	m.PushScreen(ScreenModal)
	return m.modalScreen.Init()
}
...
func (m *Model) handleServerComposeMessageMsg(msg ServerComposeMessageMsg) {
...
func (m *Model) handleHomeTrackerMsg() tea.Cmd {
	...
	return fetchTrackerList(m.prefs.Tracker)
}
